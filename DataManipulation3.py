...
del(data, digitized, kwh_bins, kwh_max, kwh_max_df,kwh_min, kwh_min_df, kwh_means, kwh_means_df)
...
#    the (normalized) total reduction of error brought by that featureb 
...
data_energy_dfB = data_energy_dfA[['WALLTYPE','ROOFTYPE','YEARMADE','REGIONC','BEDROOMS','ADQINSUL']].copy()
del(data_energy_dfA)

## Feature_Cols Based on Results from Decision Tree Model
feature_colsB = list(data_energy_dfB)
del(feature_colsA)
...
plt.hist(model_energy_df.kwh_binned, bins = 1000)
...
plt.figure()
plt.boxplot(model_energy_df.kwh_binned, 0, 'gD')
